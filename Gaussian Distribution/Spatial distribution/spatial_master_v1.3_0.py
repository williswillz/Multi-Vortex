...
x_start, x_end = 0, 50            # x-direction boundaries
...
for i in range (0, total_num):
    ...
    # xvort1[a,0] = i
    # xvort1[a,1] = 0
    vel1[:,:] = induced_velocity_single(q, Z, gam[a], core_radius[a])
#    print vel1
    a = a + 1
...
#                  density=0.5, color='black', linewidth=1, arrowsize=1, arrowstyle='->')
#pyplot.scatter(x_vortex, y_vortex, color='#CD2305', s=80, marker='o'); 
contf = pyplot.contourf(X, Y, Z, vmin=0, vmax=0.3,
                        cmap='seismic', levels=numpy.linspace(0, 0.9, 5000), extend='both')
pyplot.colorbar()
...
#pyplot.savefig('25_UV_contour_gist_earth.pdf')
pyplot.show() 