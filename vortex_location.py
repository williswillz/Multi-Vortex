...
for i in range (100, 500):
    ...
    xvort1[a,0] = i
    xvort1[a,1] = 0
    vel1[:,:] = panels.induced_velocity_single(q, xvort1, 0.24)
    a = a + 1
...
savetxt('single-vortex_array'+str()+'.csv',np.column_stack((vel1[:],vel2[:],vel3[:])), fmt='%5s', delimiter=',')
...
plt.legend(loc='upper left')
savefig('induced_velocity__location_array.pdf')
plt.show()
...
plt.legend(loc='upper left')
savefig('induced_velocity_location_tot_array.pdf')
plt.show()
