...
pts_y = np.arange(-0.1,0.1,0.01)
pts_x = (np.ones_like(pts_y))*-5
#pts_y = [0.05]
#pts_x = -1
...
p = p_ref + (0.5 * 1.225 * (v0**2 - ux**2)) #+ dphi
...
with h5py.File('p_acoustic_frequency.h5', 'w') as hf:
    hf.create_dataset("dataset",  data=mat12)
L = ((np.abs(2*mat12.real))/(2.e-5))**2
with h5py.File('p_acoustic_ref20micpa_frequency.h5', 'w') as hf:
    hf.create_dataset("dataset",  data=L)
